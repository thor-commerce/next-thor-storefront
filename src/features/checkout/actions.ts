...
export async function cartUpdateAction(input: Omit<CartUpdateInput, "cartId">) {
	...
	const resp = await getClient().mutate<CartUpdateMutation, CartUpdateMutationVariables>({
...
export async function intializePaymentSession() {
	...
	const stripePaymentGateways = mapEdgesToItems(paymentGatewaysRes.data?.paymentGateways).find(
...
export async function placeOrder({ cartId }: { cartId: string }) {
	...
	const res = await getClient().mutate<CartCompleteMutation, CartCompleteMutationVariables>({
