// import { ProductAttributeType, SwatchAttributeValue } from "@/lib/thorcommerce/storefront/generated/types.generated";
...
// 	root: TreeNode,
// ): Map<string, string[]> => {
// 	const selectedOptions = variant.selectedAttributes.map((selectedAttribute) => ({
...
// 			const optionGroup = group?.children?.find((child) => child.name === selectedOption.value);
...
// function findOrCreateChild(parent: TreeNode, name: string, type: "Group" | "Option"): TreeNode {
// 	let child = parent.children?.find((child) => child.name === name && child.type === type);
// 	if (!child) {
// 		child = {
// 			name: name,
// 			type: type,
...
// export const generateAttributeGroups = (product: ProductDetailProduct, id: string): AttributeGroup[] => {
...
// 			.sort((a, b) => {
// 				const selectedAttribute = product.attributeAssignments.find((attr) => attr.name === group.groupName);
...
// 			.flatMap(
// 				(option) =>
// 					createOption(product, selectedVariant, group, option, availableOptionsPerLevel, group.type) ?? [],
// 			);
// 	});
...
// 		attributeGroups[attributeGroups.length - 1].groupName === "Market Compliance" &&
...
// ): OptionType | null {
// 	const assignedAttribute = product.attributeAssignments.find((attr) => attr.name === group.groupName);
// 	if (!assignedAttribute || !assignedAttribute.values) return null;

// 	const value = mapEdgesToItems(assignedAttribute.values).find((x) => x.value === option);

// 	const isDisabled = !availableOptionsPerLevel.get(group.groupName)?.includes(option);
...
// 		checked: isChecked,
// 		disabled: isDisabled,
// 		src: optionType === ProductAttributeType.Swatch ? (value as SwatchAttributeValue).media?.src : null,
// 		color:
// 			optionType === ProductAttributeType.Swatch ? ((value as SwatchAttributeValue)?.color ?? null) : null,
// 	};
// }
...
// 			selectedOptions.find((option) => option.name == attr.name && option.value == attr.value),
// 		),
// 	);
...
// 		.sort((a, b) => {
// 			const aIndex = product.attributeAssignments.findIndex((x) => x.name == a.name);
// 			const bIndex = product.attributeAssignments.findIndex((x) => x.name == b.name);
// 			return aIndex - bIndex;
// 		})
...
// 	const bestVariant = variantScores!.sort((a, b) => b.score - a.score)[0].variant;
// 	return bestVariant;
// };
